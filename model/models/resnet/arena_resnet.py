...
def gen_model_config(block, layers, in_channels, image_size, num_classes,
                     kernel_sizes, norm_layer):
    return {
        "block": block,
        "layers": layers,
...
class ResNet(nn.Module):
    ...
    def log_weights(self, logger):
        ...
        weight_stats.conv_statistics(self.conv1, "conv1", logger)
    ...
    def _make_layer(self, block, channels, num_blocks, kernel_size, stride):

        layers = []
        layers.append(
            block(in_channels=self.dynamic_in_channels,
                  channels=channels,
