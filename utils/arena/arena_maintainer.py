...
class ArenaMaintainer(object):
    ...
    def model_save_dir(self):
        return self._create_dir(
            os.path.join(self.save_config["save_dir"], "models"))

    def arena_save_dir(self):
        return self._create_dir(
            os.path.join(self.save_config["save_dir"], "arena"))
    ...
    def max_weight_mutation(self):
        return self.evolution_config("max_weight_mutation") * (
            self.max_weight_mutation_decay()**self.epoch())
    ...
    def eliminate_by_accuracy(self, arena, logger):
        ...
        for arena_id, accuracy in accuracies.items():
            if accuracy < cut_off and not shielded[arena_id]:
                ...
                survive_list.append(arena_id)
        logger.log_elimination(survive_list, eliminate_list, accuracies,
                               "Performance")
        return eliminate_list
    ...
    def update_stats(self, arena):
        ...
        train_stats_dict = self._update_stats(stats_dict=train_stats_dict,
                                              arena=arena,
                                              phase=0)
        eval_stats_dict = self._update_stats(stats_dict=eval_stats_dict,
                                             arena=arena,
                                             phase=1)
        test_stats_dict = self._update_stats(stats_dict=test_stats_dict,
                                             arena=arena,
                                             phase=2)
    ...
    def _update_stats(self, stats_dict, arena, phase):
        ...
        if self.epoch() not in stats_dict["epoch"]:
            ...
            for arena_id, accuracy in accuracies.items():
                stats_dict["id: " + str(int(arena_id)) +
                           " accuracy"].append(accuracy)
                stats_dict["id: " + str(int(arena_id)) + " loss"].append(
                    losses[arena_id])
    ...
    def _load_dataframe(self, df):
        ...
        for arena_id in range(self.num_models()):
            ...
            stats_dict[col_name + " accuracy"] = df[col_name +
                                                    " accuracy"].tolist()
