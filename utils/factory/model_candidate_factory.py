...
from yyycode.utils.arena.model_candidate import ModelCandidate
...
class ModelCandidateFactory(object):
    def __init__(self, train_config, save_config):
        ...
        self.arena_save_dir = self._create_dir_if_not_exist(
            os.path.join(save_dir, "arena"))
        self.all_model_save_dir = self._create_dir_if_not_exist(
            os.path.join(save_dir, "models"))
        self.next_model_id = self.train_config["evolution_config"][
            "cur_model_id"]
    ...
    def load_model_candidate(self, arena_id):
        model_candidate_config_path = os.path.join(
            self.arena_save_dir,
            str(arena_id) + "_config.pickle")
        assert os.path.exists(
            model_candidate_config_path), "Candidate Does Not Exist In Arena"
